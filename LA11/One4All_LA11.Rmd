...
#There is really only one way to spell Andrew. However, in only cultures Andrew can be interpreted as "Ander," "Andre," etc. so we include the "rew" in brackets
...
```{r, warning = FALSE, message = FALSE}
regex1973 <- babynames %>% filter(year == 1973)
ariel1973 <- regex1973 %>% mutate(contains = str_detect(regex1973$name,"Ar+[iy]+[ae]+l")) %>% filter(contains == TRUE & sex == "F") 
...
regex1988 <- babynames %>% filter(year == 1988)
ariel1988 <- regex1988 %>% mutate(contains = str_detect(regex1988$name,"Ar+[iy]+[ae]+l")) %>% filter(contains == TRUE & sex == "F")
...
regex1990 <- babynames %>% filter(year == 1990)
ariel1990 <- regex1990 %>% mutate(contains = str_detect(regex1990$name,"Ar+[iy]+[ae]+l")) %>% filter(contains == TRUE & sex == "F")
...
ariel1989 <- regex1989 %>% mutate(contains = str_detect(name, "Ar+[iy]+[ae]+l")) %>% filter (contains == TRUE & sex == "F") 
...
megan1988  <- regex1988 %>% mutate(contains = str_detect(name, "Meg+[ha]+n")) %>% filter (contains == TRUE & sex == "F") 
megan1989  <- regex1989 %>% mutate(contains = str_detect(name, "Meg+[ha]+n")) %>% filter (contains == TRUE & sex == "F")  
