...
sumflights <- DENflights %>% group_by(MONTH) %>% count()
sumdelayed <- DENflights %>% filter(ARR_DELAY >= 15) %>% group_by(MONTH) %>% count

ggplot(sumflights) + geom_col(mapping = aes(x = 2*MONTH-1, y = n, fill = n, position = "dodge", width = .75)) + ggtitle("Total of Flights in each Month, Including delayed Flights") + geom_col(data = sumdelayed, aes(x = 2* MONTH, y = n, fill = n, position = "dodge", width = .75)) + labs(y = "Number of Flights", x = "Month")

```

# Findings

  It seems that there isn't a strong affect on the probability of having a delayed flight controlling for the month. While it looks like in June, July and August there are more delayed flights per overall flights in those months, it is not significantly higher than the rest of the year. Also, the delayed flights in January seem to be at a slightly higher rate than other months.
  
  
# Recommendation

While you have to fly when you have to fly, it can be assumed, based on the above graph, that the probability of your flight to or from Denver has a higher probability of being delayed in January, or over the summer. However, this is not a strong correlation and the probabilities of a delayed flight only change slighty from month to month.


# Does flight duration have an effoct on the probability of any given flight to be delayed?

```{r, setup = FALSE, warning = FALSE, message = FALSE}
totalflights <- DENflights %>% group_by(AIR_TIME <= 180) %>% count()
delayedflights <- DENflights %>% filter(ARR_DELAY >= 15) %>% group_by(AIR_TIME <= 180) %>% count()

ggplot(totalflights) + geom_col(aes(x = FALSE, y = n, fill = n, position = "dodge")) + geom_col(aes(x = TRUE, y = n, fill = n, position = "dodge")) + ggtitle("Long Flights vs. Short Flights") + geom_col(data = delayedflights, aes(x = FALSE, y = n, fill = n, position = "dodge")) + geom_col(data = delayedflights, aes(x = TRUE, y = n, fill = n, position = "dodge"))

```
# Findings:

There seems to be no effect on the probability of a flight being delayed based on the duration of the flight. This graph is split into 2 bars, the first bar being flights less than 3 hours and the second bar being flights over 3 hours. The Data seems to line up almost exactly. 

# Recommendation:

It can be asssumed from the findings that the duration of a flight has no effect on the probability of a flight being delayed, so do not assume that your flight will have a higher chance of being delayed if it is a long, or short flight.
