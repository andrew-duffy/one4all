...
## Question:
**Does the use of an actual question for the title have any effect on the score?**
This is an interesting question to me because it seems like a random question unlikely to result in a correlation, but it would be cool if it does. People could view titles with full questions in the names as more relatable to exactly what they are looking for and would therefore be more likely to upvote the answer.

```{r}
questions <- questions %>% mutate(question = str_detect(Title,"\\?$"))
qtrue <- questions %>% filter(str_detect(Title,"\\?$"))
tavg <- summarize(qtrue,mean(Score))
tt <- tally(qtrue, name = "Score")
qfalse <- questions %>% filter(!str_detect(Title,"\\?$"))
favg <- summarize(qfalse,mean(Score))
ft <- tally(qfalse, name = "Score")
```


```{r}
ggplot(questions) + geom_bar(mapping = aes(x = question)) + geom_jitter(mapping = aes(x = question, y = Score, color = question)) + labs(title = "Use of a Question vs. Score", x = "Score", y = "Use of Question")
```

## Observations:

From the plot above, it can be concluded that having a question as the title will result in a higher score. The histogram in the plot represents the total count of all the scores for both true and false. It is obvious that while false definitely has a larger total count, the average for true is higher. This tells us that while there were more titles that were not questions, those titles that were questions had higher average scores. The mean score for false was 18.87 and the mean score for true was 34.95 which is signficantly higher. This finding supports my hypothesis that using a question will usually result in more upvotes.
...
## Question regarding Questions file: Since these are questions regarding python, how do the scores vary with posts with the word "Python" in the title? Do all highscoring posts have "python" in the title? Also, what about the appearance of the language python is based off of, "C++", in the title?
...
```
# Observations: 
As seen in the plots above, there seems to be a higher chance of a post becoming popular when there are "Python" or "C++" in the title. However, this cannot be immediately deduced as a correlation because there are many factors going into this. Questions with "Python" in the title I would assume to be more general questions that novice users are asking (in the python section of stackoverflow, there is no reason to include python in the title), which have a larger pool of people searching for that question. Therefore, these posts can become more popular due to that.

#Note: 
I filtered for score > 10 because it just fills the bottom of the graph less, and there are already so many posts with very low scores.



## Question regarding the Answers file: How is the score of a response affected by the length of the text?
```{r, echo = FALSE, warning = FALSE}
...
It seems that there is too much data to draw a conclusion on how length affects score of a response. Not to mention that there seems to be high-scoring posts at every length up to 4,000 characters. However, there is a small concentration of high-scoring posts in which their lengths are very close to 0. I would assume this means simple answers to simple questions get the highest results compared to answers that complicate a simple question and provide a lengthy response. 


# Consolodation:

From the findings above it can be seen that posts that are too long or short, and posts with words that are too long or too short tend not to achieve higher scores, also mentioning a programming language in the title also tends to increase the score, and having a question mark in the title of a question. Thus the ideal question post would have a length between 0 and 1000 characters, as well as a mean word length of around 5, mention a specific programming language and have a question mark. For answers, it would have a simmilar mean word length and include the word python in the body as well as in the title. Additionally for answers, they should be posted within around half an hour of the questions being posted. 

The overall scores distribution seems to either not be a normal distribution, or one that is very skewed. It seems that there are too many posts with a score of 0 or very close to 0, which skews results greatly. Howevever, it is still possible to calculate percentiles since you can do that with any data distribution, and as long as the data has a mean and a standard distribution the z-score can still be calculted, however for non-normal or very skewed data it might not be a particularly useful statistic, due to the fact there are many more posts close to a score of 0. It is hard to find a good way to benefit from the results considering posts with very low scores can still be very helpful, epecially to those asking very advanced questions in which the pool of people searching for that question, and the pool of people with the knowledge to respond are both very small compared to simple questions that anyone with a basic knowledge of python can answer.

This data could help stack overflow in examining what type of posts tend to be favored by their users, and they could filter for this in order to make it easier for their users to find good posts. This could hurt those with questions they need help with, but that don't lie in the higher scoring post characteristics. Even though these posts don't tend to get high scores, they can get useful answers, and it would be more difficult to get these answers if stack overflow filtered/sorted the posts based on what they found that users prefered. 

Stack overflow could also benefit from this knowledge by changing their search algorithms in ways to help all users. For example, it can match searches for advanced questions not by popularity, but by proximity of the question search and the text of the already existing question post. But for simple questions, stack overflow should match searches with similar popular posts to get an answer that is the most helpful. 
...
**Andrew:** I examined the relevance of "Python" and "C++" appearing in the title of posts and popularity. I did this by mutating the original dataframe to create a boolean variable whether or not "Python" or "C++" was a string vector in the title. Then, I graphed that variable controlling for score to see the results. While my findings were not the best, there does seems to be more popular posts with either coding language in the title. I also examined the length of an answer to its score to see if longer or shorter responses yield higher scores. I did this using str_count and creating a new variable in the answers dataframe to show the length of the response in the graph. The results again were quite skewed towards 0, even after filtering out scores of 0.

**Ahyo:** For this lab I looked at how the usage of questions in the title affects the score of the answer. To do this i used a geom_bar function behind a geom_jitter function to show the density of the plot as well as the total count. I subsequently found that there is a correlation between whether or not a question is used and the score. If a question is used, the average score is higher than if no question is used.
