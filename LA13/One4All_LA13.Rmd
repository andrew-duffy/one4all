...
### Question:
What is the average difference in percentage of the country covered in forest between the largest $CO_2$ emittor per capita and the smallest? Additionally, is the data statistically significant?

### Importance:
This question is important because it can emphasize just how much damage $CO_2$ emissions cause. By using emissions per capita instead of total emissions, confounding variables like total population can be ruled out. If there is found to be a substantial difference in percentage, it can be concluded that $CO_2$ emissions per capita and forest coverage. This result can also lead to the conclusion that deforestation is related to individual emissions, pointing to the importance of individual emission reductions.

### Hypotheses:
Null hypothesis: There will be no substantial difference in the two countries' forest coverage.

Other hypothesis: There will be a significant difference in forest coverage between the largest emittors and the smallest.

```{r}
#First, determine which country has the smallest emissions per capita on average
cpp <- co2_per_person %>% group_by(country) %>% mutate(average = mean(co2_emissions_tonnes_per_person))
largest <- cpp %>% arrange(desc(average)) %>% filter(country != "Qatar")
smallest <- cpp %>% arrange(average) #Chad: avg = 0.030440

#Next, find the difference in forest coverage of those countries
fc <- forest_coverage %>% filter(country == "Kuwait" | country == "Chad")
mg <- tibble(mean=perm_mean(1000,fc$forest_coverage_percent,24))

#Using the map function to compute the mean of forest coverage percent
k <- fc %>% filter(country == "Kuwait") %>% select(forest_coverage_percent) %>% map(mean)
c <- fc %>% filter(country == "Chad") %>% select(forest_coverage_percent) %>% map(mean)
amd <- k$forest_coverage_percent-c$forest_coverage_percent #actual average found to be -4.552

#Plot of histogram
ggplot(data=mg,aes(mean,fill=(mean<quantile(mean,probs = c(.025),na.rm=T)[[1]]|mean>quantile(mean,probs = c(.975),na.rm=T)[[1]])))+
  geom_histogram() + geom_vline(xintercept = abs(amd), color = "blue") + labs(title = "Mean Difference Between Chad and Kuwait", x = "Percent forest", fill = "tails") 
```

### Findings
It is apparent from the above plot that the null hypothesis can be safely rejected. This can be concluded because the actual mean difference is far outside of the histogram plot in the 100th percentile showing there is most likely statistic significance. This proves that there is a large difference in the country percentage covered by forests between the top and botom emittors per capita. If the emissions per person in each country were to be lowered, it can be concluded that it could greatly improve the state of forests around the world.
...
## Subquestion: 
It is commonly known that most of the world's polution comes from Asia. Although that is true for polution of the ocean, is it also true for co2 emissons?

Null Hypothesis: Polution is equally distrubited throughout all of the populated continents

Test statistic: Mean of yearly co2 emissions for Asia - mean for yearly co2 emissions for all other continents

```{r}

real <- co2_data %>% filter(year == 2014) %>% select(yearly_co2_emissions_1000_tonnes,continent) %>% group_by(continent)

realasia <- real %>% filter(continent == "asia")

realAM <- real %>% filter(continent == "americas")

realEU <- real %>% filter(continent == "europe")

realAFR <- real %>% filter(continent == "africa")

meanAS <- mean(realasia$yearly_co2_emissions_1000_tonnes, na.rm = TRUE)

meanother <- mean(mean(realAM$yearly_co2_emissions_1000_tonnes, na.rm = TRUE) + mean(realEU$yearly_co2_emissions_1000_tonnes, na.rm = TRUE) + mean(realAFR$yearly_co2_emissions_1000_tonnes, na.rm = TRUE))
```

Before the Permutation test, we should find the real difference between co2 emissions in Asia vs. the rest of the world.

The difference is:

```{r, echo = FALSE}
realdiff <- meanAS - meanother

realdiff
```

meaning that actually, Asia produces less co2 emissions (on average) than the rest of the world. 

Now that we can reject the null hypothesis, we sould represent the difference as a positive number so it can be represented on the histogram for the permutation test. We willl make it negative again later.  Therefore:

```{r}
realdiff = meanother - meanAS
...
```{r}
real <- real %>% arrange(continent, .by_group = TRUE)
```

```{r}
realpm <- c(real$yearly_co2_emissions_1000_tonnes)

permtibble <- perm_mean(perms = 1000, realpm, 56)
...
```{r}
permtibble <- as.tibble(permtibble)
ggplot(data = permtibble) + geom_histogram(aes(x = value, binwidth = .2, color = "gray")) + geom_vline(xintercept = -(realdiff), color = "red") + ggtitle("Permutation Test of Means of co2 Emissions by Continent") + labs(x = "co2 Emissions, in 1000 tons")

```

## Conclusion:
It is clear that the null hypothesis should be rejected, as the graph leans very negative, meaning that Asia on average has a lot less co2 emissions than the rest of the world. This is a lot different of a result than I was expecting, although I did expect the null hypothesis to be rejected. However, I expected the null hypothesis to be rejected due to Asia having much higher co2 emissions than the rest of the world, but instead Asia had a significantly lower emissions rate than the rest of the world, on average.
