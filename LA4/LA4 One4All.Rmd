...
splt <- split(babies, babies$smoke)

nonsmokers <- splt[[1]]
smokers <- splt[[2]]

numvars <- colnames(babies)
length <- length(numvars)

for (i in 1:length){
  smoking_corr <- cor(smokers$gestation, smokers[i], use = "complete.obs")
  nonsmoking_corr <- cor(nonsmokers$gestation, nonsmokers[i], use = "complete.obs")
  corr_diff <- nonsmoking_corr - smoking_corr
  list[i] <- corr_diff
}

d <- ggplot()
d <- d + geom_bar(aes(numvars, list))
print(d)
```
